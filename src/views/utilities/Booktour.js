...
import {
  handleUpdate_StatusTickets,
  handleGetTickets,
  handleCustommers,
  handleVouchers,
  handleGetUsers,
  handleConfirmCancleTicket,
  ...
  return (
    <Paper sx={{ p: 3, borderRadius: 2, boxShadow: 2 }}>
        ...
        <TableBody>
          {ticket?.map((ticket, index) => (
            <TableRow key={ticket.id}>
              ...
              <TableCell sx={{ fontSize: "13px", fontFamily: "Arial" }}>
                <Typography>
                  <strong>Điểm đi:</strong> {ticket.Departure_Location}
                  <br /> <br />
                  <strong>Điểm đến:</strong> {ticket.Destination}
                  <br />
                  <br />
                  <strong>Ngày khởi hành:</strong>{" "}
                  {ticket?.Departure_Date?.slice(0, 10)}
                  <br />
                  <br />
                  <strong>Giờ khởi hành:</strong> {ticket.Departure_Time}
                  <br />
                  <br />
              ...
              <TableCell sx={{ fontSize: "13px", fontFamily: "Arial" }}>
                {ticket?.Created_at_Booking?.slice(0, 10)}
              </TableCell>
              <TableCell sx={{ fontSize: "13px", fontFamily: "Arial" }}>
                <Chip
              ...
              <TableCell sx={{ fontSize: "40px", fontFamily: "Arial" }}>
                <FormControl style={{ minWidth: "130px", minHeight: "50px" }}>
                  ...
                  >
                    {ticket.Status_Payment !== "Chưa Thanh Toán" &&
      ...
      <Dialog open={openBell} onClose={handleCloseBell} maxWidth="md" fullWidth>
        <DialogTitle style={{ backgroundColor: "#3fd0d4", color: "#fff" }}>
        ...
        >
          <List>
            {ticket
              ?.filter((ticket) => ticket.isRequestCancel === true)
              .map((ticket, index) => (
                <ListItem
                  key={index}
                  sx={{
                ...
                >
                  {users
                    ?.filter((user) => user._id === ticket.id_user)
                    .map((user) => (
                      <div key={user._id}>
                        <h4>{user.Name}</h4>
                        <Avatar
      ...
      <Dialog open={openBill} onClose={handleClose}>
        <DialogTitle sx={{ fontFamily: "Arial", textAlign: "center" }}>
          Hóa Đơn
        </DialogTitle>
        <DialogContent sx={{ fontFamily: "Arial" }}>
          {selectedBill && (
            <Grid container spacing={2}>
              {valueCus &&
                valueCus.map((cus, index) => (
                  <Grid item xs={12} key={index}>
                    <Typography variant="h6" sx={{ fontFamily: "Arial" }}>
                      Thông Tin Khách Hàng
                    </Typography>
                    <Typography>
                      <strong>Tên:</strong> {cus?.Name_Customer}
                    </Typography>
                    <Typography>
                      <strong>Ngày Sinh:</strong> {cus?.Date_Of_Birth}
                    </Typography>
                    <Typography>
                      <strong>Giới Tính:</strong> {cus?.Sex_Customer}
                    </Typography>
                    <Typography>
                      <strong>Số Điện Thoại:</strong> {cus?.Phone_Number}
                    </Typography>
                    <Typography>
                      <strong>CCCD:</strong> {cus?.Citizen_Identification}
                    </Typography>
                    <Typography>
                      <strong>Địa Chỉ:</strong> {cus?.Address}
                    </Typography>
                  </Grid>
                ))}
              <Grid item xs={12}>
                <Typography variant="h6" sx={{ mt: 2, fontFamily: "Arial" }}>
                  Chi Tiết Vé
                </Typography>
                <Typography>
                  <strong>Giá Vé Gốc:</strong> {selectedBill.Price_Tour} VND
                </Typography>
                <Typography>
                  <strong>Giá Vé Sau Khi Giảm:</strong>{" "}
                  {selectedBill.After_Discount > 0
                    ? selectedBill.After_Discount
                    : selectedBill.Price_Tour}{" "}
                  VND
                </Typography>
                <Typography>
                  <strong>Số Lượng Người Lớn:</strong> {selectedBill.Adult}
                </Typography>
                <Typography>
                  <strong>Số Lượng Trẻ Em:</strong> {selectedBill.Children}
                </Typography>
                {valueVou &&
                  valueVou.map((voucher) => (
                    <Typography key={voucher._id}>
                ...
                <Typography>
                  ...
                  {selectedBill.Adult + selectedBill.Children}
                </Typography>
                <Typography>
                  <strong>Giá Vé Người Lớn :</strong>{" "}
                  {selectedBill.After_Discount > 0
                    ? selectedBill.Adult_fare + "( Đã bao gồm giảm giá)"
                    : selectedBill.Adult_fare}
                </Typography>
                <Typography>
                  <strong>Giá Vé Trẻ Em:</strong>{" "}
                  {selectedBill.After_Discount > 0
                    ? selectedBill.Children_fare + "( Đã bao gồm giảm giá)"
                    : selectedBill.Children_fare}
                </Typography>
                <Typography>
                  <strong>Tổng Giá:</strong> {selectedBill.Total_price} VND
                </Typography>
                <Typography>
                  <strong>Mã Voucher:</strong> {selectedBill?.id_Voucher}
                </Typography>
                <Typography>
                  <strong>Tổng Giá Sau Giảm Giá:</strong>{" "}
                  {selectedBill.Total_price} VND
                </Typography>
                <Typography>
                ...
                <Typography>
                  ...
                  {selectedBill.Created_at_Booking.slice(0, 10)}
                </Typography>
                <Typography>
                  <strong>Thời Gian Tạo:</strong>{" "}
                  {selectedBill.Created_at_Booking.slice(11, 16)}
                </Typography>
              </Grid>
            </Grid>
          )}
